...
public final class CreativeTabNetworkBridge {
    ...
    public static boolean sendToPlayer(ServerPlayer player, CustomPacketPayload payload) {
        ...
        if (sender == null) {
            OneEnoughCreativeTabConstants.LOGGER.error("Cannot send {} to player: server network bridge is not installed", payload.type().id());
            return false;
        }
        ...
        try {
            ...
            OneEnoughCreativeTabConstants.LOGGER.error("Failed to send {} to player {}", payload.type().id(), player.getScoreboardName(), exception);
            return false;
        }
    }
    ...
    public static boolean sendToServer(CustomPacketPayload payload) {
        ...
        if (sender == null) {
            OneEnoughCreativeTabConstants.LOGGER.error("Cannot send {} to server: client network bridge is not installed", payload.type().id());
            return false;
        }
        ...
        try {
            ...
            OneEnoughCreativeTabConstants.LOGGER.error("Failed to send {} to server", payload.type().id(), exception);
            return false;
        }
    }
