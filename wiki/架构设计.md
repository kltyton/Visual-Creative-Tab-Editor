...
2. 捕获 Fabric/NeoForge 事件及其他 Mod 最终贡献。
...
- 保存等待服务端确认；取消直接丢弃预览。
- Fabric 和 NeoForge 分别使用各自原生分页状态，公共编辑器通过薄平台接口切换页面。
...
- Fabric：`visual_creative_tab_editor.classtweaker`。
- NeoForge：公共 `META-INF/accesstransformer.cfg` 加 NeoForge 专用 AT。
